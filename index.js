...
if (typeof GRAPHITE_ENDPOINT == 'undefined' || typeof GRAPHITE_TOKEN == 'undefined') {
  console.error('Make sure environment variables (GRAPHITE_ENDPOINT & GRAPHITE_TOKEN) are set!')
  process.exit(1)
}
...
const ns = (s) => s.toString().toLowerCase().replace(/[\W_]+/g, "_");
...
axios
  ...
  .then((response) => {
    ...
    table.find("tbody > tr").each((_, row) => {
      ...
      const entry = {
        name: generateMetricName(STATE, ZONE, data["Town"], data["Company"]),
        interval: 10,
        time: TIME,
  ...
  .then((tableData) => axios.post(GRAPHITE_ENDPOINT, tableData, {
      headers: {
